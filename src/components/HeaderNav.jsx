...
export const HeaderNav = ({ }) => {
    ...
    useEffect(() => {
        const storedYear = localStorage.getItem('activeYear');
        if (storedYear !== null) {
            setActiveYear(Number(storedYear));
        } else {
            setActiveYear(0);
        }
    }, []);
    ...
    const selectYear = (index) => {
        setActiveYear(index);
        localStorage.setItem('activeYear', index);
        console.log('Year selected:', years[index]);
    };
